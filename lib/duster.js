...
module.exports.init = function(options, logger, preProcessors) {
    ...
    if (!hasProp(options, 'subDirSeparator')) options.subDirSeparator = '__';
    if (!hasProp(options, 'compileAll')) options.compileAll = false;
    if (!hasProp(options, 'compiledAllFilename')) options.compiledAllFilename = 'all';
        ...
        fs.readFile(file, function(err, data) {
            ...
            fs.writeFile(outputFile, compiled, function (err) {
                if (err) {
                    ...
                    if (compileAll) {
                        fs.appendFile(compileAllOutputFile, (compiled + '\n'));
                    }
                }
            });
        });
