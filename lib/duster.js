...
module.exports.init = function(logger, preProcessors) {
    ...
    fs.readFile(configFile, 'utf8', function(err, data) {
        if (err) {
            ...
            compiler(JSON.parse(data), logger, preProcessors);
        }
    });
};

function compiler(config, logger, preProcessors) {
    var inputDir = path.normalize(cwd + config.raw_dir),
        outputDir = path.normalize(cwd + config.pre_dir),
        realTimeCompiling = config.real_time || true;
